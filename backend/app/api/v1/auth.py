...
async def request_verification_code(
    request: RequestVerificationCode, db: Session = Depends(get_db)
):
    """
    ...
    message, masked_email = verification_service.request_verification_code(
        request.verification_token
    )

    return VerificationCodeResponse(
        success=True, message=message, email_masked=masked_email
    )
...
async def verify_email(request: VerifyEmail, db: Session = Depends(get_db)):
    ...
    access_token, refresh_token, user_dict = verification_service.verify_email(
        verification_token=request.verification_token, code=request.code
    )
...
async def reset_password(request: PasswordResetRequest, db: Session = Depends(get_db)):
    ...
    return PasswordResetResponse(
        success=True, message="Password successfully reset", user=user_data
    )
